...
type Service interface {
	...
	GetHistoryTransaksiByUserID(ID int) (User, error)
...
func (s *service) Register(input InputRegister) (User, error) {
	...
	user.LegalName = input.LegalName
...
func (s *service) GetHistoryTransaksiByUserID(ID int) (User, error) {
	user, err := s.repository.FindTransaksiByUserID(ID)
	if err != nil {
		return user, err
	}
...
func (s *service) CreatePhoto(ID int, namefotoKTP string, namefotoSelfie string) (User, error) {
	user, err := s.repository.CreatePhoto(ID, namefotoKTP, namefotoSelfie)
	if err != nil {
		return user, err
	}
